...
class User(db.Model, UserMixin):
    ...
    def is_duplicate_username(self):
        query = User.query.filter_by(username=self.username)
        # 만약 self.id가 존재한다면 (즉, 이미 데이터베이스에 저장된 사용자라면)
        # 자기 자신은 중복 체크에서 제외해야 함
        if self.id:
            query = query.filter(User.id != self.id)
...
class UsageLog(db.Model):
    ...
    prediction_result = db.relationship('PredictionResult', back_populates='usage_logs') # PredictionResult 관계
def __repr__(self) -> str:
...
class PredictionResult(db.Model):
    ...
    usage_logs = db.relationship("UsageLog", back_populates="prediction_result")  # 변경 사항: UsageLog 모델과의 관계 추가
